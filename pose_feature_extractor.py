...
class PoseFeatureExtractor:
    ...
    def __init__(self, feature_points, segment_length,norm_factor):
    ...
    def get_pair_feature(self, video_pose_segments, feature_paramaters):
        ...
        for segments in video_pose_segments:
            temp_feature = []

            # get dist between target points from all frames
            abs_all, diff_all = PoseFeatureExtractor.point_pair_distances(joint_1, joint_2, segments)#,
            ...
            if abs_diff_flag:
                ...
                #print("Normalization factor ", lecture_pose_segments.norm_factor)

            temp_feature = np.hstack(temp_feature)
            if self.norm_factor >= 0.1:
                temp_feature = np.hstack(temp_feature) / self.norm_factor
    ...
    def get_point_feature(self, video_segments, feature_paramaters):
        ...
        for segment_data in video_segments:
            ...
            point_info = PoseFeatureExtractor.get_point_info(test_target, segment_data)#,
                                                             #lecture_pose_segments.point_dim)
            #print("POINT DIM", lecture_pose_segments.point_dim)
    ...
    @staticmethod
    def get_point_info(joint, points_data):


        p0 = points_data[:, joint*2:joint*2+2]
